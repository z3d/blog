...
The report also checks compiled shape and source vocabulary, but the basic loop is simple. Pick a cohort of files that solve the same kind of problem. Pin a few examples that show the intended shape. Compare the rest of the cohort with those examples, then report which files look furthest away and why.
