...
The implementation uses Ledoit-Wolf shrinkage as a guardrail. In practical terms, the code blends the covariance estimated from the exemplars with a scaled identity matrix before inverting it. When the shrinkage weight is high, the score behaves closer to standardised distance over z-scored features; when it is lower, the feature relationships in the exemplars have more influence. I needed the models to help me get to that design, and then the reports made it clear whether the result was useful.

That guardrail keeps the distance finite and conservative when the exemplar set is small. It does not make the number a gate, and it does not remove the need for the per-feature report.
...
The shingle layer took a different approach by dropping operands entirely and comparing the remaining opcode patterns with the exemplar opcode patterns. I wanted this to catch the "same feature vector, different control flow" case. It did that sometimes, but it often moved with structural distance. I would keep it as a cheap second opinion, not build it before the structural and per-feature reports.
